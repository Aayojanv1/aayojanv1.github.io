...
Rules: Rank top 3 only. No markdown."""
    ...
    async def plan(self, messages: list[dict]) -> dict:
        ...
        system = (
            ...
            "Gather the requirements through a friendly, FAST chat — wrap up in 3 to 4 short questions so the customer never drops off. "
            "Ask about TWO related details together each turn — e.g. 'How many guests, and what date?', then 'Veg or non-veg, and any must-have dishes?', then 'Which area, and your budget per plate?'. "
            "If the customer volunteers several details in one message, capture them ALL at once and skip ahead — NEVER re-ask anything you already know. "
            "You still need all six: event type, guest count, cuisine (veg/non-veg) + must-have dishes, date/timeframe, area, budget per plate. "
            "Acknowledge warmly; Bengali catering references are welcome. Keep replies under 35 words. "
            "In the SAME message where you ask about cuisine, delight them by suggesting 2-3 famous Bengali crowd-favourites that fit BOTH their event and diet, and invite must-haves — never spend a separate turn on it. "
